...
class ExponentialFamily(abc.ABC):
    ...
    def log_pdf(self, x: jnp.ndarray, params: jnp.ndarray) -> jnp.ndarray:
        ...
        linear_term = self.sufficient_statistics(x)[...,None,:] @ natural_parameters[...,None]
        log_partition = self.log_partition(natural_parameters)
        return linear_term[...,0,0] - log_partition + log_base_measure

    def conjugate_log_partition(self, alpha: ArrayLike | jnp.array, nu: ArrayLike | jnp.array, /) -> jnp.array:
        """Signature `(P),()->()`
    ...
    def posterior_parameters(self, prior_natural_params: ArrayLike | jnp.ndarray, data:ArrayLike | jnp.ndarray) -> jnp.ndarray:
        """Signature `(P),(D)->(P)`
        """
        ...
        return jnp.append(
            alpha_prior + expected_sufficient_statistics,
            nu_prior + n
        )
...
class ConjugateFamily(ExponentialFamily):
    ...
    def sufficient_statistics(self, likelihood_canonical_params: ArrayLike | jnp.ndarray, /) -> jnp.ndarray:
        """Signature `(D)->(P)`
    ...
    def unnormalized_log_pdf(self, likelihood_canonical_params: ArrayLike | jnp.ndarray, natural_params: ArrayLike | jnp.ndarray, /) -> jnp.ndarray:
        """Signature `(D),(P)->()`
        """
        return self.sufficient_statistics(likelihood_canonical_params)@jnp.asarray(natural_params)

    def laplace_precision(self, natural_params: ArrayLike | jnp.ndarray, mode: ArrayLike | jnp.ndarray, /) -> jnp.ndarray:
        """Signature `(P),(D)->()`
        """
        return -1*jax.hessian(self.unnormalized_log_pdf, argnums=0)(jnp.asarray(mode), natural_params)